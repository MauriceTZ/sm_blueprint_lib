...
shapeset_json_path = os.path.join(GAME_DATA, "Objects", "Database", "ShapeSets", "lights.json")
print(shapeset_json_path)
survival_shapeset_json_path = os.path.join(SURVIVAL_DATA, "Objects", "Database", "ShapeSets", "lights.json")
...
all_parts = parts_json["partList"] + survival_fittings_json["partList"]
...
f'''
@dataclass
class {b["name"].replace(" ", "").replace("-", "").replace("'", "").replace(":", "_")}(BaseInteractablePart):
