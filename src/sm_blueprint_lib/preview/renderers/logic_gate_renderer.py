...
class LogicGateRenderer:
    ...
    # def add_from_blueprint(self, blueprint: Blueprint):
    #     self.instances.clear()
    #     # self.instances = {}
    #     for body in blueprint.bodies:
    #         for block in body.childs:
    #             if block.shapeId == CLASS_TO_SHAPEID[LogicGate]:
    #                 block: LogicGate
    #                 if block.controller.id in self.instances:
    #                     raise ValueError(
    #                         f"ID DUPLICATION!!!\n{block}\nand\n{self.instances[block.controller.id]}")
    #                 self.instances[block.controller.id] = block
    #     if not self.instances:
    #         return

    #     models = np.array([self.get_model(logicgate).to_list()
    #                        for logicgate in self.instances.values()], dtype="f4")
    #     # self.models = self.context.buffer(models)
    #     self.models.orphan(models.nbytes)
    #     self.models.write(models)

    #     colors = np.array([self.get_color(logicgate).to_list()
    #                       for logicgate in self.instances.values()], dtype="f4")
    #     # self.colors = self.context.buffer(colors)
    #     self.colors.orphan(colors.nbytes)
    #     self.colors.write(colors)

    #     states = np.array([self.get_state(logicgate)
    #                       for logicgate in self.instances.values()], dtype="f4")
    #     # self.states = self.context.buffer(states)
    #     self.states.orphan(states.nbytes)
    #     self.states.write(states)

    #     types = np.array([self.get_type(logicgate)
    #                      for logicgate in self.instances.values()], dtype="f4")
    #     # self.types = self.context.buffer(types)
